...
class ConnectController extends Controller
{
    ...
    public function connexionForTestChunk($request, $response, $arg)
    {
        $help = new Helper();
        $state = json_decode($_GET['state']);
        $permission_test = $state->permission;
        ...
        if(isset($accessToken)) {
            ...
            if($if_additionnal_info == 0)
              $url = 'https://'.$lang.'.'.$this->base_domain.'/chunkstart/'.$id;
            else
              $url = 'https://'.$lang.'.'.$this->base_domain.'/chunkstart/1/'.$id;
